...
signType=[1];for(g=0;hash.length+signType.length+2<tag;)signType.push(255),g++;signType.push(0);for(unpacked=-1;++unpacked<hash.length;)signType.push(hash[unpacked]);signType=new Buffer(signType);hash=BN.mont(key.modulus);sig=(new BN(sig)).toRed(hash);sig=sig.redPow(new BN(key.publicExponent));sig=new Buffer(sig.fromRed().toArray());key=8>g?1:0;tag=Math.min(sig.length,signType.length);sig.length!==signType.length&&(key=1);for(unpacked=-1;++unpacked<tag;)key|=sig[unpacked]^signType[unpacked];return 0===
key}}
